...
export default function SIPPage() {
  ...
  const handleDelete = async (id) => {
    ...
    try { await api.delete(`/sips/${id}`); toast.success('SIP deleted'); load(); }
                    ...
                    <tr key={sip._id} onClick={() => setViewingAsset(sip)} style={{ cursor: 'pointer' }} className="hover-row">
                      ...
                      <td onClick={e => e.stopPropagation()}>
                        <div style={{ display: 'flex', gap: 4 }}>
                          ...
                          <button className="btn btn-ghost btn-icon btn-sm" onClick={() => handleDelete(sip._id)}><Trash2 size={14} /></button>
                        </div>
                      </td>
                    </tr>
                  );
            ...
            <div className="modal" onClick={e => e.stopPropagation()}>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="form-row">
                  <div className="form-group"><label className="form-label">Family Member *</label><select className="form-select" value={form.memberId} onChange={e => setForm({ ...form, memberId: e.target.value })} required>{members.map(m => <option key={m._id} value={m._id}>{m.avatar} {m.name}</option>)}</select></div>
                  <div className="form-group"><label className="form-label">Category</label><select className="form-select" value={form.category} onChange={e => setForm({ ...form, category: e.target.value })}>{SIP_CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                </div>
                <div className="form-row">
                ...
                <div className="form-row">
                  ...
                  <div className="form-group"><label className="form-label">Status</label><select className="form-select" value={form.status} onChange={e => setForm({ ...form, status: e.target.value })}><option value="active">Active</option><option value="paused">Paused</option><option value="completed">Completed</option></select></div>
                </div>
